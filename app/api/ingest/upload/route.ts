...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { error: uploadError } = await supabase.storage.from('victory_screenshots').upload(storagePath, buffer, {
    ...
    if (uploadError) {
      ...
      if ((uploadError as any).statusCode === '409') {
        ...
        return NextResponse.json({ error: 'Duplicate screenshot', email: 'k1zm3ths@gmail.com' }, { status: 409 })
      }
